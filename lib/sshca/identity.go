...
func (i *Identity) Encode(certFormat string) (*ssh.Certificate, error) {
	...
	if len(i.AllowedResourceAccessIDs) != 0 {
		allowedResourceAccessIDsBytes, err := types.ResourceAccessIDsToBytes(i.AllowedResourceAccessIDs)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		cert.Permissions.Extensions[teleport.CertExtensionAllowedResourceAccessIDs] = string(allowedResourceAccessIDsBytes)
	}
...
func DecodeIdentity(cert *ssh.Certificate) (*Identity, error) {
	...
	if v, ok := takeExtension(teleport.CertExtensionAllowedResourceAccessIDs); ok {
		resourceAccessIDs, err := types.ResourceAccessIDsFromBytes([]byte(v))
		if err != nil {
