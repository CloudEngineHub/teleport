...
// intended to reach a kube cluster through a relay. The prefix in front should
// be the encoded hash generated by [SNIPrefixForTarget].
...
func (d *PassiveForwarder) Dispatch(serverName string, transcript *bytes.Buffer, rawConn net.Conn) (dispatched bool) {
	encodedHash, ok := strings.CutSuffix(serverName, SNISuffix)
	if !ok {
		return false
	}

	go d.forward(encodedHash, transcript, rawConn)
	return true
}

func (d *PassiveForwarder) forward(encodedHash string, transcript *bytes.Buffer, clientConn net.Conn) {
	ctx := d.ctx
	log := d.log.With(
	...
	encodedHash = strings.ToLower(encodedHash)
	if len(encodedHash) != encodedHashLen {
		log.DebugContext(ctx,
			"Received bad SNI for kube relay forwarding",
	...
	if _, err := base32hex.Decode(desiredHash[:], []byte(encodedHash)); err != nil {
		log.DebugContext(ctx,
			"Received malformed SNI for kube relay forwarding",
			"encoded_hash", encodedHash,
		)
		return
	}
