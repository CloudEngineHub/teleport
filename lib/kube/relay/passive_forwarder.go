...
type PassiveForwarder struct {
	...
	// resolvedNames contains (hash, name) pairs for which aaaaaaaaaaaaaaaaaa
...
// resources are gone.
func (d *PassiveForwarder) Close() error {
	d.mu.Lock()
...
func (d *PassiveForwarder) Dispatch(serverName string, transcript *bytes.Buffer, rawConn net.Conn) (dispatched bool) {
	sniPrefix, ok := strings.CutSuffix(serverName, SNISuffix)
	if !ok {
		return false
	}

	go d.forward(sniPrefix, transcript, rawConn)
	return true
}

func (d *PassiveForwarder) forward(sniPrefix string, transcript *bytes.Buffer, clientConn net.Conn) {
	ctx := d.ctx
	log := d.log.With(
	...
	kubeClusterName, cached := d.resolvedNames.Get(desiredHash)
	kubeServers, err := d.getKubeServersWithFilter(
		ctx,
		func(ks readonly.KubeServer) bool {
			if ks.GetRelayGroup() != d.groupName {
				return false
			}
			...
			if desiredHash == hashForTarget(d.clusterName, ks.GetCluster().GetName()) {
				kubeClusterName = ks.GetCluster().GetName()
				if !cached {
					d.resolvedNames.Add(desiredHash, kubeClusterName)
				}
				return true
			}
			return false
	...
	for kubeServer := range healthcheck.OrderByTargetHealthStatus(kubeServers) {
		target := kubeServer.GetHostID() + "." + d.clusterName

		localTunnelConn, err := d.localDial(ctx,
		...
		peerTunnelConn, err := d.peerDial(ctx,
			target, apitypes.KubeTunnel,
	...
	if ctx.Err() != nil {
		d.mu.Unlock()
		// abandon ship, the forwarder was closed while dialing
		return
	}
	d.wg.Add(1)
	d.mu.Unlock()
	...
	wg.Go(func() {
		defer log.LogAttrs(ctx, logutils.TraceLevel,
