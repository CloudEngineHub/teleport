...
cluster](./get-started/deploy-community.mdx) with Teleport Community Edition,
migrate to [Teleport Enterprise (Cloud)](./get-started/deploy-cloud.mdx) to roll out
Teleport across your organization, and deploy a
[self-hosted](zero-trust-access/deploy-a-cluster/deploy-a-cluster.mdx) Teleport Enterprise cluster
...
1. If you are migrating to a self-hosted Teleport Enterprise account, plan and
   ...
   Teleport](zero-trust-access/deploy-a-cluster/deploy-a-cluster.mdx).
...
1. We recommend that Teleport Enterprise (Self-Hosted) customers set up automatic updates to maintain a healthy set of Agents, and require this in Teleport Enterprise (Cloud). See how to
   [Set up Automatic Agent Updates](upgrading/agent-managed-updates/agent-managed-updates.mdx) to
...
Review the [dynamic resources](reference/infrastructure-as-code/teleport-resources/teleport-resources.mdx) list to see if any
...
1. For each Agent and Machine ID Bot, obtain a valid join token. We recommend
   ...
   method](reference/deployment/join-methods.mdx).
...
read the [full documentation](machine-workload-identity/deployment/deployment.mdx) on deploying a
Machine ID Bot.
...
account](./get-started/deploy-cloud.mdx).

Read the documentation on [Self-Hosting Teleport
Enterprise](zero-trust-access/deploy-a-cluster/deploy-a-cluster.mdx).
