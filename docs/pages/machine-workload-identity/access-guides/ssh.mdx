...
## Step 2/3. Configure the `tbot` output service

Now, `tbot` needs to be configured with a service that will produce an SSH
certificate and OpenSSH configuration. For this we use the `identity` output
service type.
...
To use `tsh` with a `tbot` identity service the path to the `identity` file
...
To use OpenSSH with the identity service, the path to the `ssh_config` should be
