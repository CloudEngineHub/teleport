...
In addition, the `format` field in the database output service controls the
format of the generated credentials. This allows for compatibility with clients
that expect a specific format. When this field is not specified, a sensible
...
`directory` destination will be used. This will write artifacts to a specified
directory on disk. Ensure that this directory can be written to by the Linux
user that `tbot` runs as, and that it can be read by the Linux user that will be
